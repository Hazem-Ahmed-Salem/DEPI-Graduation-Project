...
stop_button = st.button("Stop",on_click=set_button_clicked)


while cap.isOpened():

    ret,frame = cap.read()
    if not ret or st.session_state.button_clicked :
    ...
    overlay = frame.copy()
    cv.rectangle(overlay, (0, 0), (300, 200), (0, 0, 0), -1)
    cv.rectangle(overlay, (frame.shape[1]-300, 0), (frame.shape[1], 200), (0, 0, 0), -1)
    ...
    if results.multi_hand_landmarks:
        ...
        for idx, (hand_landmarks, hand_info) in enumerate(zip(results.multi_hand_landmarks, handedness)):
            ...
            if confidence > 70:
                ...
                cv.putText(frame, text, (x_pos, 40), 
                          cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv.putText(frame, f"{confidence:.1f}%", (x_pos, 70), 
                ...
                for pred_i in sorted_idx:
                    ...
                    if conf > 10:  # Only show if confidence > 10%
                        ...
                        cv.putText(frame, pred_text, (x_pos, y_offset), 
